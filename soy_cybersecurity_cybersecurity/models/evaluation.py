...
class Result(models.Model):
    ...
    alternative = fields.Many2one(
        comodel_name="cyber_evaluation.criterio.line",
        string="Alternativa",
        domain="[('criterio_id','=',criterio_id)]",
    )

    alternative_description = fields.Text(
        related="alternative.description", string="Descripción de la alternativa"
    )
    ...
    def _compute_text_values(self):
        ...
        if alternative:
            text_values = "El valor mínimo es %s y el máximo es %s" % (
                alternative.value_less,
                alternative.value_high,
            )
    ...
    def _onchange_value(self):
        if (
            ...
            or self.value > self.alternative.value_high
        ):
            msg = "El valor tiene que estar entre %s y %s" % (
                self.alternative.value_less,
                self.alternative.value_high,
            )
            raise UserError(msg)
    ...
    def _constrains_value(self):
        for each in self:
            if (
                each.value < each.alternative.value_less
                or each.value > each.alternative.value_high
            ):
                msg = "El valor tiene que estar entre %s y %s" % (
                    each.alternative.value_less,
                    each.alternative.value_high,
                )
                raise UserError(msg)
            if each.value == 0:
